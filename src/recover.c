  ...
  *mapped_block = 0UL;
  ...
  if (last_direct) {
	 ...
	 * offset starts at 0, so always add 1 to offset/ZONES_PER_BLOCK */
      ...
      blknr +=
        ZONES_PER_BLOCK; /* There were ZONES_PER_BLOCK indirect index blocks we skipped over */
    }
    if (fsc->X3_INDIRECT) {
    ...
    if (blknr < (ZONES_PER_BLOCK * ZONES_PER_BLOCK)) {
      ...
      if (block == 0UL) {
        *mapped_block = fsc->N_DIRECT + ZONES_PER_BLOCK * (ZONES_PER_BLOCK + 1);
        ...
        ind = cache_read_block(block, NULL, CACHEABLE);
        block = block_pointer(
          ind, (unsigned long)(blknr / ZONES_PER_BLOCK), fsc->ZONE_ENTRY_SIZE);
        if (block == 0UL) {
          /* 2 here, 1 for indirect block, 1 for current 2xindirect */
          *mapped_block =
            fsc->N_DIRECT + ZONES_PER_BLOCK * (2 + blknr / ZONES_PER_BLOCK);
          /* *mapped_block = fsc->N_DIRECT + ZONES_PER_BLOCK*2 + blknr; */
          return (EMB_2IND_L1_ZERO);
        } else if (block < sb->nzones) {
          ind = cache_read_block(block, NULL, CACHEABLE);
          *mapped_block = block_pointer(ind,
            (unsigned long)(blknr % ZONES_PER_BLOCK),
            fsc->ZONE_ENTRY_SIZE);
          if (*mapped_block >= sb->nzones) {
            *mapped_block = 0UL;
    ...
    if (blknr < (ZONES_PER_BLOCK * ZONES_PER_BLOCK * ZONES_PER_BLOCK)) {
      ...
      if (block == 0UL) {
        *mapped_block = fsc->N_DIRECT +
                        ZONES_PER_BLOCK * (ZONES_PER_BLOCK + 1) +
        ...
        ind = cache_read_block(block, NULL, CACHEABLE);
        block = block_pointer(ind,
          (unsigned long)(blknr / (ZONES_PER_BLOCK * ZONES_PER_BLOCK)),
          fsc->ZONE_ENTRY_SIZE);
        ...
        if (block == 0UL) {
          ...
          *mapped_block = fsc->N_DIRECT + ZONES_PER_BLOCK +
                          ZONES_PER_BLOCK * ZONES_PER_BLOCK *
                            (2 + blknr / (ZONES_PER_BLOCK * ZONES_PER_BLOCK));
        ...
        *mapped_block = fsc->N_DIRECT +
        ...
        static char warn_once =
          0; /* Only warn once about not using 3x indirects */
        *mapped_block = 0UL;
        if (!warn_once) {
          lde_warn(
            "Triple indirects are ignored: Recompile with -DBETA_CODE to handle triple indirect blocks");
          warn_once = 1;
        }
#endif
      }
  ...
  long increment)
{
  ...
  if ((FS_cmd.map_block(zone_index, *ipointer, &blknr)) ||
      (blknr != (*currblk))) {
...
int recover_file(int fp, unsigned long zone_index[], unsigned long filesize)
{
  ...
  while (1) {
    ...
    if (lde_flags.quit_now) {
      lde_warn(
        "User aborted recovery, partial data may have been written to file.");
      return -2;
    }
...
#warning MSDOS HACK ENABLED
      for (k = 0; k < sb->zonesize; k++) { /* MSDOS HACK */
        ...
        if (write(fp, dind, write_count) != write_count) {
          lde_warn(
            "Write error: unable to write block (%ld) to recover file, recover aborted",
            nr);
          return -1;
        }
...
int check_recover_file(unsigned long zone_index[], unsigned long filesize)
{
  ...
  while (1) {
    ...
    if ((nr) && FS_cmd.zone_in_use(nr)) {
      lde_warn(
        "Block %ld (0x%lX) in use. (q=abort, l=lkupinode, c=cont)", nr, nr);
      result = mgetch();
      if (result == 'q') {
        ...
        while (1) {
          ...
          if (lookedup == 0UL) {
            lde_warn(
              "Can't find an inode referencing block %ld (0x%lX).  Hit any key to continue.",
              nr,
              nr);
            (void)mgetch();
            break;
            ...
            lde_warn(
              "Block %ld (0x%lX) is used by inode %ld (0x%lX).  Hit any key to continue search ('q' to quit).",
              nr,
              nr,
              lookedup,
              lookedup);
  ...
  unsigned long start_nr)
{
  ...
  for (/*nr set above*/; nr < sb->nzones; nr++) {
          ...
          if (lde_flags.inode_lookup) {
            if ((inode_nr = find_inode(nr, 0UL))) {
              ...
              if (lde_flags.check_recover) {
                ...
                fprintf(stderr,
                  ", recovery %spossible",
                  ((check_recover_file(GInode->i_zone, GInode->i_size) == 0)
                      ? ""
                      : "NOT "));
	 ...
	 */
        ...
        match[0] = dind[1];
        for (i = (fsc->ZONE_ENTRY_SIZE - 1); i < sb->blocksize / 4;
             i += fsc->ZONE_ENTRY_SIZE) {
          no_match_flag = 1;
	   ...
	   */
          ...
          if (match_count[2] > SEQ_COUNT) {
            fprintf(stderr,
              "%d hits on possible indirect block at 0x%lX\n",
              match_count[2],
              nr);
            match_total++;
          }
  ...
  for (; sbnr < sb->nzones; sbnr += rs) {
    ...
    if ((match = memmem(buffer, bytesread, searchstring, lsearchstring))) {
      ...
      *moffset = ((int)(match - buffer)) % sb->blocksize;
      lde_warn("matches block=0x%lx offset=0x%lx",
        *mbnr,
        *moffset + sbnr * sb->blocksize);
      retval = 1;
      break;
    }
    if (lde_flags.quit_now) {
...
int search_for_superblocks(int fs_type)
{
  ...
  if (fs_type >= LAST_AUTO_FSTYPE || fs_type < AUTODETECT) {
    lde_warn(
      "Bad filetype specified (i.e. why would \"no\" have a superblock) . . . Aborting.");
    return -1;
  }

  lde_warn("Searching disk for %s superblocks . . .",
    (fs_type == AUTODETECT) ? "any" : lde_typedata[fs_type].name);
  ...
  for (; sbnr < sb->nzones; ++sbnr) {
    ...
    if (fs_type == AUTODETECT) {
      ...
      if (lde_typedata[fs_type].test(buffer, 0)) {
        lde_warn(
          "Found %s superblock at 0x%lx", lde_typedata[fs_type].name, sbnr);
      }
    }
