...
static int  cwrite_block(cached_block *this_block, bm_cursor *curs, 
                         bm_flags *flags);
static void highlight_helper(int row, int col, bm_cursor *curs, 
			     bm_flags *flags);
...
static void cdump_block(unsigned long nr, bm_cursor *curs)
{
  ...
  while ((j<curs->wl)&&(j*curs->rs+curs->sow<curs->eod)) {
	...
	mvwprintw(workspace,j,38+i*3,"%2.2X",curs->data[j*16+i+8+curs->sow]);
    }

    for (i=0; ((i<curs->rs)&&((j*curs->rs+i+curs->sow)<curs->eod)); i++) {
  ...
  if (flags->modified) {
    ...
    for (;;) {
      ...
      c = cquery("WRITE OUT BLOCK DATA TO DISK "
		 ...
		 "ydcf",(lde_flags.write_ok)?"":warning);
...
static void highlight_helper(int row, int col,
			     bm_cursor *curs, bm_flags *flags)
{
  unsigned char c;
  int s_col;
				 ...
				 bm_cursor *curs, bm_irecord *iptr)
{
  ...
  if (iptr) {
    ...
    c = advance_zone_pointer(iptr->inode->i_zone,
			     &new_bnr,&(iptr->inr),
			     iptr->increment);
  ...
  *passed_block = this_block;
  ...
  for (i=1; i<=curs->bps; i++) {
    ...
    if (iptr) {
      other_bnr = new_bnr;
      if (advance_zone_pointer(iptr->inode->i_zone,
			       &other_bnr,&(iptr->inr),-i) )
	other_bnr = sb->nzones;
    } else {
      other_bnr = new_bnr-i;
    }
...
#     endif /* BETA_CODE */
	...
	if ((calc_offset(&curs)>=curs.eob)&&(this_block->next->size)) {
	  if (update_block_data(&this_block,this_block->next->bnr,
				 irecptr, &curs, &flags, curs.sow-curs.eob)) {
	    curs.col--;
	    break;
	  }
	}
	...
	irecord.increment = 0;
	if (!update_block_data(&this_block,this_block->bnr,
			       &irecord, &curs, &flags, 0)) {
	  irecptr = &irecord;
	  irecord.increment = (c==CMD_NEXT_IND_BLOCK)?+1:-1;
	  clear_data_cache(this_block,&curs);
	  update_block_data(&this_block,this_block->bnr,
			    &irecord, &curs, &flags, 0);
	...
	if (update_block_data(&this_block,current_block,
			      irecptr, &curs, &flags, 0) ) {
	  current_block = this_block->bnr;
	  break;
	}
 	flags.edit_block = curs.col = curs.row = 0;
	...
	flags.modified = flags.ascii_mode = flags.edit_block = 0;
	update_block_data(&this_block,current_block,
			  irecptr, &curs, &flags,
			  (-curs.sob + curs.sow));
	...
	if ( (search_iptr = find_inode(current_block, search_iptr)) ) {
	  lde_warn("Block is indexed under inode 0x%lX. "
		   " Repeat to search for more occurances.",search_iptr);
	...
	if (cwrite_block(this_block,&curs,&flags))
	  break;
	temp_ptr = 
	  block_pointer(&(curs.data[calc_offset(&curs)]),
