...
static struct
{ /* Someday I'll clean up dump_inode() to use something like this */
  int sb_entry;
  int row;
  int col;
  char *text;
  ...
  int park_row;
  int park_col;
} inode_labels[] = { { I_MODE, 2, 0, "TYPE: ", "type", 2, 6 },
...
static void cwrite_inode(unsigned long inode_nr,
  struct Generic_Inode *GInode,
  int *mod_yes);
static void cdump_inode_labels(void);
static void cdump_inode_values(unsigned long nr,
  struct Generic_Inode *GInode,
  int highlight_field);
static void set_inode_field(int curr_field,
  unsigned long a,
  struct Generic_Inode *GInode);
static void limit_inode(unsigned long *inode, struct sbinfo *local_sb);
static void parse_edit(WINDOW *workspace,
  int c,
  int *modified,
  struct Generic_Inode *GInode,
  int highlight_field,
  ...
  int *mod_yes)
{
  ...
  if (*mod_yes) {
    while (
      (c = cquery("WRITE OUT INODE DATA TO DISK [Y/N]? ",
         "ynfq",
         lde_flags.write_ok
           ? ""
           : "(NOTE: write permission not set on disk, use 'F' to set flags before 'Y')")) ==
      'f') {
      flag_popup();
    }
...
#if HAVE_GETPWUID
    mvwprintw(
      workspace, 4, 10, "(%s)", (NC_PASS != NULL) ? NC_PASS->pw_name : "");
#endif
...
#if HAVE_GETGRGID
    mvwprintw(
      workspace, 4, 30, "(%s)", (NC_GROUP != NULL) ? NC_GROUP->gr_name : "");
#endif
  ...
  struct Generic_Inode *GInode)
{
  switch (curr_field) {
    case I_MODE:
      GInode->i_mode =
        (GInode->i_mode & 07770000) + ((unsigned short)a & 07777);
      break;
    case I_MODE_FLAGS:
      beep();
      GInode->i_mode =
        (GInode->i_mode & 07777) + (((unsigned short)a & 017) * (07777 + 1));
      break;
    case I_UID:
...
void parse_edit(WINDOW *workspace,
  int c,
  int *modified,
  struct Generic_Inode *GInode,
  int highlight_field,
...
int inode_mode()
{
  ...
  cdump_inode_values(
    current_inode, GInode, (highlight_field | LDE_DUMP_ILABELS));
  ...
  while ((c = next_cmd) || (c = lookup_key(mgetch(), inodemode_keymap))) {
    ...
    switch (c) {
      ...
      case CMD_EDIT: /* Edit inode */
        parse_edit(
          workspace, c, &modified, GInode, highlight_field, park_x, park_y);
        break;

      case CMD_VIEW_AS_DIR: /* View inode as a directory */
        if (S_ISDIR(GInode->i_mode) &&
            ((highlight_field >= I_ZONE_0) && (highlight_field <= I_ZONE_LAST)))
          if (GInode->i_zone[highlight_field - I_ZONE_0]) {
            directory_popup(
              0UL, current_inode, (unsigned long)(highlight_field - I_ZONE_0));
            GInode = FS_cmd.read_inode(current_inode);
            cdump_inode_values(
              current_inode, GInode, (highlight_field | LDE_DUMP_ILABELS));
          }
      ...
      case CMD_COPY: /* Copy inode to copy buffer */
        if (!copy_buffer)
          copy_buffer = malloc(sizeof(struct Generic_Inode));
      ...
      case CMD_PASTE: /* Paste inode from copy buffer */
        if (copy_buffer) {
          ...
          if (!lde_flags.write_ok)
            lde_warn("Turn on write permissions before saving this inode");
        } else {
      ...
      case CMD_NUMERIC_REF: /* Go to an inode specified by number */
        if (ncread("Enter inode number (leading 0x or $ indicates hex):",
              &a,
              NULL)) {
          current_inode = a;
          limit_inode(&current_inode, sb);
