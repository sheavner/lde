...
static unsigned long DOS_map_inode(unsigned long ino)
{
  ...
  block = ino * (unsigned long)fsc->INODE_SIZE / sb->blocksize +
          2 * sb->zonesize; /* WHY 2*zonesize? */
                            /*  if (block > sb->ninodes) */
                            /*   block = 0; */
  return block;
}
...
static int DOS_dir_entry(int i, lde_buffer *block_buffer, lde_dirent *d)
{
  ...
  if (((long)i) * sb->dirsize >= block_buffer->size) {
    ...
    d->inode_nr = (unsigned long)ldeswab16(dir->start) +
                  (((unsigned long)ldeswab16(dir->starthi)) << 16);
    ...
    DOS_junk_inode.i_ctime =
      date_dos2unix(ldeswab16(dir->ctime), ldeswab16(dir->cdate));
    DOS_junk_inode.i_atime = date_dos2unix(0, ldeswab16(dir->adate));
    DOS_junk_inode.i_mtime =
      date_dos2unix(ldeswab16(dir->time), ldeswab16(dir->date));
...
static int DOS_map_block(unsigned long zone_index[],
  unsigned long blknr,
...
static void DOS_sb_init(void *sb_buffer)
{
  ...
  sb->first_data_zone = ldeswab16(Boot->reserved) + sb->zmap_blocks -
                        2 * sb->zonesize; /* * Boot->cluster_size */
  sb->max_size = 1;
  sb->magic = 0;
