...
static int dump_dir_entry(WINDOW *win,
  int i,
  int off,
  lde_buffer *block_buffer,
  int highlight);
static void highlight_dir_entry(WINDOW *win,
  int nr,
  int *last,
  int screen_off,
  lde_buffer *buffer);
static void redraw_dir_window(WINDOW *win,
  int max_entries,
  int screen_off,
  lde_buffer *buffer);
...
static lde_menu dp_help[] = { { CMD_EXPAND_SUBDIR,
                                "expand directory under cursor" },
  { CMD_EXPAND_SUBDIR_MC,
    "expand directory under cursor and make it the current inode" },
  { CMD_INODE_MODE, "make inode under cursor the current inode" },
  { CMD_INODE_MODE_MC,
    "make inode under cursor the current inode and view it" },
#ifdef JQDIR
  ...
  { CMD_COPY_TO_FILE,
    "Copies directory name and files contents to new location " },
#endif /* JQDIR */
  { 0, NULL } };

/* default keymap for directory mode */
static lde_keymap dirmode_keymap[] = { { 'i', CMD_SET_CURRENT_INODE },
  { 'I', CMD_INODE_MODE_MC },
  { 'Q', CMD_EXIT },
  { LDE_ESC, CMD_EXIT },
  { 'd', CMD_EXPAND_SUBDIR },
  ...
  int highlight)
{
  ...
  if (!FS_cmd.dir_entry(i + off, block_buffer, &d))
    return 0;
  inode_nr = d.inode_nr;
  ...
  if (inode_nr) {
    ...
    mvwprintw(
      win, i, 0, "  0x%8.8lX: %24s %-*s", inode_nr, " ", COLS - 40, fname);
  }
...
static int reread_dir(lde_buffer *block_buffer, unsigned long bnr)
{
  ...
  if (bnr)
    memcpy(block_buffer->start,
      cache_read_block(bnr, NULL, CACHEABLE),
      block_buffer->size);
...
static int get_inode_info(unsigned long inode_nr, lde_buffer *buffer)
{
  ...
  while (count < GInode->i_size) {
    ...
    if (bnr)
      count += (unsigned long)nocache_read_block(
        bnr, ((buffer->start) + c * sb->blocksize), lookup_blocksize(bnr));
    c++;
  }
  ...
  while ((c = lookup_key(mgetch(), dirmode_keymap))) {
    ...
    switch (c) {
      ...
      case CMD_NEXT_SCREEN: /* Next screen w/scroll */
        ...
        for (c = 0; c < VERT / 2; c++) {
          if ((current < (VERT - 1)) &&
              ((current + screen_off) < max_entries)) {
            current++;
          } else if ((current == (VERT - 1)) &&
                     ((current + screen_off) < max_entries)) {
            last--;
            wscrl(win, 1);
