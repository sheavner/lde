...
unsigned long EXT2_map_inode(unsigned long ino)
{
  ...
  block =
    ((ino - 1) % sb->s_inodes_per_group) / (sb->blocksize / fsc->INODE_SIZE);
...
#endif
    if (nocache_read_block(
          group_desc[i].bg_inode_bitmap, local_map, bytes_read) != bytes_read)
...
static void EXT2_read_block_bitmap(unsigned long nr)
{
  ...
  for (i = 0; i < group_desc_count; i++) {
    ...
    if (nocache_read_block(group_desc[i].bg_block_bitmap,
          local_map,
          sb->s_blocks_per_group / 8) != sb->s_blocks_per_group / 8)
...
static void EXT2_read_tables()
{
  ...
  group_desc_count =
    (sb->nzones - sb->first_data_zone + sb->s_blocks_per_group - 1) /
...
static unsigned long EXT2_next_entry(struct ext2_dir_entry_2 *bp, void *end)
{
  static struct ext2_dir_entry_2 *tag_bp =
    0; /* this is set to if we called as part of a
						 linear dir read */
  static struct ext2_dir_entry_2 *save_entry_sz =
    0; /* Place to store entry size
  ...
  if (in_del && (tag_bp->inode > sb->ninodes ||
                  ...
                  tag_bp->name_len < 1 ||
                  tag_bp->name_len >
                    sb->namelen)) { /* bad inode possibly reading garbage */
    entry_size = ldeswab16(bp->rec_len); /*  */
...
int EXT2_dir_check_entry(int entry, lde_buffer *buffer)
{
  ...
  if (!EXT2_dir_entry(entry, buffer, &d))
    return 0;
