...
#define MSDOS_MAX_EXTRA                                                        \
  3 /* tolerate up to that number of clusters which are
...
#define ATTR_DIR_READ_BOTH                                                     \
  512 /* read both short and long names from the
...
#define IS_FREE(n)                                                             \
  (!*(n) || *(const unsigned char *)(n) == DELETED_FLAG ||                     \
...
#define MSDOS_CAN_BMAP(mib)                                                    \
  (!(((mib)->cluster_size & 1) || ((mib)->data_start & 1)))
...
extern int date_dos2unix(uint16_t time, uint16_t date);
...
extern void fat_date_unix2dos(int unix_date, uint16_t *time, uint16_t *date);
extern int fat__get_entry(struct inode *dir,
  loff_t *pos,
  struct buffer_head **bh,
  struct msdos_dir_entry **de,
  int *ino);
static __inline__ int fat_get_entry(struct inode *dir,
  loff_t *pos,
  struct buffer_head **bh,
  struct msdos_dir_entry **de,
  int *ino)
...
extern int fat_scan(struct inode *dir,
  const char *name,
...
extern struct inode *fat_build_inode(struct super_block *,
  struct msdos_dir_entry *,
  int,
  int *);
extern struct super_block *fat_read_super(struct super_block *s,
  void *data,
  int silent,
  struct inode_operations *dir_ops);
...
extern int fat_search_long(struct inode *dir,
  const char *name,
  int len,
  int anycase,
  loff_t *spos,
  loff_t *lpos);
extern int fat_readdir(struct file *filp, void *dirent, filldir_t);
extern int fat_dir_ioctl(struct inode *inode,
  struct file *filp,
  unsigned int cmd,
  unsigned long arg);
...
extern int msdos_rename(struct inode *old_dir,
...
extern int vfat_rename(struct inode *old_dir,
  struct dentry *old_dentry,
  struct inode *new_dir,
  struct dentry *new_dentry);
extern struct super_block *vfat_read_super(struct super_block *sb,
  void *data,
  int silent);
