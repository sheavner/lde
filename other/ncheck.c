...
void do_ncheck(int argc, char **argv)
{
  ...
  if (!iw.iarray) {
    com_err("do_ncheck", ENOMEM, "while allocating inode info array");
    return;
  }
  ...
  while (ino) {
    ...
    retval = ext2fs_dir_iterate(fs, ino, 0, 0, ncheck_proc, &iw);
    if (retval) {
  ...
  for (i = 0, iinfo = iw.iarray; i < iw.num_inodes; i++, iinfo++) {
    if (iinfo->parent == 0)
      continue;
    retval =
      ext2fs_get_pathname(fs, iinfo->parent, iinfo->ino, &iinfo->pathname);
    if (retval)
      com_err("ncheck",
        retval,
  ...
  for (i = 0, iinfo = iw.iarray; i < iw.num_inodes; i++, iinfo++) {
    ...
    printf("%ld\t%s\n",
      iinfo->ino,
      iinfo->pathname ? iinfo->pathname : "<unknown pathname>");
    if (iinfo->pathname)
      free(iinfo->pathname);
  }
