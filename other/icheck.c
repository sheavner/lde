...
int icheck_proc(ext2_filsys fs, blk_t *block_nr, int blockcnt, void *private)
{
...
void do_icheck(int argc, char **argv)
{
  ...
  while (ino) {
    ...
    retval = ext2fs_block_iterate(fs, ino, 0, block_buf, icheck_proc, &bw);
    if (retval) {
      com_err("icheck", retval, "while calling ext2_block_iterate");
      goto next;
    }
